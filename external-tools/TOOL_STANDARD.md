...
Guide for building external tools for aitower. The reference
...
```
tool-name/
  manifest.json        # aitower metadata (required)
...
```bash
# Install
git clone <repo> ~/Workspace/aitower/external-tools/tool-name
cd ~/Workspace/aitower/external-tools/tool-name
python3 -m venv .venv && .venv/bin/pip install -r requirements.txt  # if Python
# Set up config/credentials as needed
tool-name login

# Uninstall
rm -rf ~/Workspace/aitower/external-tools/tool-name
```
